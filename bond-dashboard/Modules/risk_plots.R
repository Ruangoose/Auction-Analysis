...
generate_var_distribution_plot <- function(data, params = list()) {
    ...
    # =========================================================================
    # SANITIZE RETURNS: Remove NA/NaN/Inf from scaled_return before density
    # estimation. The first observation per bond after lag() returns NA, and
    # any degenerate yield data can produce NaN/Inf in the return calculation.
    # =========================================================================
    n_before_sanitize <- nrow(returns_data)
    returns_data <- returns_data %>%
        filter(
            !is.na(scaled_return),
            !is.nan(scaled_return),
            !is.infinite(scaled_return)
        )
    ...
    # =========================================================================
    # MINIMUM OBSERVATION THRESHOLD FOR DENSITY ESTIMATION
    # ggridges::stat_density_ridges calls findInterval() which requires sorted,
    # non-NA vectors from the kernel density estimate. Bonds with very few
    # observations produce degenerate density estimates that crash findInterval().
    # Require at least 30 return observations for stable density estimation.
    # =========================================================================
    min_obs_for_density <- 30
    bond_obs_count <- returns_data %>%
        group_by(bond) %>%
        summarise(n_obs = n(), .groups = "drop")

    sufficient_bonds <- bond_obs_count %>%
        filter(n_obs >= min_obs_for_density) %>%
        pull(bond)

    insufficient_bonds <- bond_obs_count %>%
        filter(n_obs < min_obs_for_density) %>%
        pull(bond)
    ...
    # Use an adaptive threshold: bonds with sd < 10% of median sd are excluded.
    # =========================================================================
    bond_variance <- returns_data %>%
        group_by(bond) %>%
        summarise(ret_sd = sd(scaled_return, na.rm = TRUE), .groups = "drop")

    median_sd <- median(bond_variance$ret_sd, na.rm = TRUE)
    min_variance_for_density <- max(0.05, median_sd * 0.10)  # At least 0.05%, or 10% of median
    ...
    if (length(low_variance_bonds) > 0) {
        message(sprintf("[VaR Distribution] Excluding %d bond(s) with near-zero return variance (sd < %.3f%%, median sd = %.3f%%): %s",
                        length(low_variance_bonds), min_variance_for_density, median_sd,
    ...
    if (nrow(returns_data) == 0 || length(sufficient_bonds) == 0) {
        ...
        return(
            ggplot() +
                annotate("text", x = 0.5, y = 0.5,
                         label = paste0("No bonds have enough return observations\n",
                                       "for density estimation (minimum: ", min_obs_for_density, ").\n",
    ...
    if (is.null(bond_order)) {
        ...
        # Filter pre-specified bond_order to only include bonds still in var_levels
        # (some may have been excluded due to insufficient observations)
        available_bonds <- unique(as.character(var_levels$bond))
        bond_order <- bond_order[bond_order %in% available_bonds]
    }
    ...
    if (use_ridgeline) {
        p <- tryCatch({
            ...
            # Prepare VaR line data with matching factor levels
            var_segments <- var_levels %>%
                mutate(bond_ridge = factor(bond, levels = var_order))
            ...
            p_ridge <- ggplot(returns_data, aes(x = scaled_return, y = bond_ridge, fill = after_stat(x))) +
                ggridges::geom_density_ridges_gradient(
                    ...
                    gradient_lwd = 0.3
                    # quantile_lines removed: triggers findInterval() crash on low-variance bonds
                ) +
                # Add VaR reference lines from precomputed var_levels (no findInterval)
                geom_segment(
                    data = var_segments,
                    aes(x = VaR_95, xend = VaR_95,
                        y = as.numeric(bond_ridge) - 0.3,
                        yend = as.numeric(bond_ridge) + 0.3),
                    color = "#FF9800", linetype = "dashed", linewidth = 0.8,
                    inherit.aes = FALSE
                ) +
                geom_segment(
                    data = var_segments,
                    aes(x = VaR_99, xend = VaR_99,
                        y = as.numeric(bond_ridge) - 0.3,
                        yend = as.numeric(bond_ridge) + 0.3),
                    color = "#D32F2F", linetype = "solid", linewidth = 0.8,
                    inherit.aes = FALSE
                ) +
                ...
                labs(
                    ...
                    subtitle = paste0("Ordered by 95% VaR | Orange dashed = 95% VaR | Red solid = 99% VaR | ",
            ...
            warning(sprintf("[VaR Distribution] Ridgeline construction error: %s", e$message))
            use_ridgeline <<- FALSE
            NULL
        })

        if (is.null(p)) use_ridgeline <- FALSE
    }
