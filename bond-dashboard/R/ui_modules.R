...
# ================== REPORT OPTIONS MODULE ==================

#' Report Configuration Panel
#' @param id Module ID
report_options_ui <- function(id) {
    ns <- NS(id)

    tagList(
        h4("Report Configuration", class = "text-primary"),

        textInput(
            ns("title"),
            "Report Title:",
            value = paste("SA Government Bonds Analysis -",
                          format(Sys.Date(), "%B %Y")),
            width = "100%"
        ),
        ...
        checkboxGroupInput(
            ...
            selected = c("exec", "market", "value", "signals"),
            width = "100%"
        ),
        ...
        conditionalPanel(
            condition = paste0("input['", ns("generate"), "'] > 0"),

            wellPanel(
                style = "background-color: #d4edda; border: 1px solid #c3e6cb;",

                h5("Report Ready!", class = "text-success"),
